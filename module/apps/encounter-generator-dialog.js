    ...
    return tableArray
        ...
        funcELCalc(monsterArray){
            ...
            //this function converts CR to (CR/2)^2 so we can combine all the numbers then bring them back down with log2*2
                ...
                return arr.map(function (x) { if(x>2){
...
getMonsters() {
    ...
    while (j < limit) {
        if(breakOut){break;} //this is to break out of the loop whenever we are done adding monsters
        ...
        if (EL < targetEL) {
            ...
            while (testCount <= testLimit) {
                ...
                monsters.forEach(monster => {
                    ...
                    testELArray.push({ id: monster.resultId, name: monster.text, img: monster.img, cr: monsterCR.item.data.details.cr, HP: monsterCR.item.data.attributes.hp.max, AC: monsterCR.item.data.attributes.ac.normal.total, SPD: monsterCR.item.data.attributes.speed.land.base })
            ...
            if(monsterArray==0){return ui.notifications.error("No monsters could be rolled, is Target EL lower than lowest table CR??")}
...
for (let [key, value] of counts) {
    for(i of filteredArr){
        if(i.id===key){
            countArray.push({id:i.id,count:value,name:i.name,cr:i.cr,img:i.img,AC:i.AC,HP:i.HP,SPD:i.SPD,count:value})
...
var textnode = document.createTextNode("EL:"+ELText);
node.appendChild(textnode);
document.getElementById("ELGoesHere").appendChild(node)
}
