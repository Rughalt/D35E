...
function execFunctions(obj) {
  for (let key in obj) {
    if (typeof obj[key] === "object") execFunctions(obj[key]);
    else if (typeof obj[key] === "function") obj[key] = obj[key]();
  }
}
    ...
    if (item.id) {
      try {
        ...
        if (item.consumableType) {
          ...
          if (consumableItem.data._id) delete data._id;
          ...
          it.data.data.enhancements.items =  it.data.data.enhancements.items || [];
          for (let enhancement of enhancements) {
            ...
            it.data.data.enhancements.items.push(
              enhancementData["data.enhancements.items"].splice(-1)[0]
            );
          }
      ...
      switch (magicItemData.type) {
        case "item":
          ...
          if (magicItemData.roll && magicItemData.roll !== "1d1") {
            let ItemAmount = rollDice(magicItemData.roll, options.enable3DDice);
            if (forceRolls && forceRolls.length > 0) {
          ...
          if (magicItemData.valueRoll) {
            Object.assign(result, {
              value:
                result.value +
                rollDice(magicItemData.valueRoll, options.enable3DDice),
            });
          }
          return result;
        case "roll":
          ...
          Object.assign(result, {
            value: result.value + (magicItemData.value || 0) + valueBonus,
            enhancement:
              result.enhancement || 0 + magicItemData.enhancement || 0,
          });
          let extraOverride = {
            data: {
              data: {
        ...
        case "ammunition":
          ...
          return this.rollItem(table, grade, prefix, forceRolls, options);
        case "extraItem":
          let extraItem = {
            ...
            amount: rollDice(magicItemData.roll, options.enable3DDice),
            ability: [],
            enhancement: 0,
            id: magicItemData.id,
          };
          extraItem.itemOverride = {
            data: {
              data: {
                price: Math.floor(extraItem.value / extraItem.amount),
                masterwork: options.masterwork,
              },
            },
          };
        ...
        case "rollScroll":
          ...
          let scrollAmountRoll = rollDice(amountFormula, options.enable3DDice);
          if (forceRolls && forceRolls.length > 0) {
        ...
        case "roll+":
          //item roll
          roll = this.rollItem(table, grade, prefix, forceRolls, options);
          ...
          for (let ability of abilityRoll) {
            if (
              roll.ability.filter((ab) => ab.itemType === ability.itemType)
                .length === 0
            ) {
              Object.assign(result, {
          ...
          Object.assign(result, {
            ...
            ability: JSON.parse(JSON.stringify(abilities.concat(roll.ability))),
            table: roll.table,
            id: roll.id,
        ...
        case "ability":
          ...
          if (magicItemData.table) {
            let { itemTypeExtra, idOverride, itemOverride } = this.rollItem(
              magicItemData.table,
              grade,
              prefix,
              forceRolls,
    ...
    TreasureLevels.forEach((TreasureLevel) => {
      ...
      if (treasureRow.extraItems) {
        ...
        times(extraItemsNo).forEach(() => {
          try {
            this._addItem(
              this.rollItem(MagicItemTable, "major", "", ItemRollFudge, {
                identified: identified,
                // TODO are potions rings etc ok to be masterwork as well?
                masterwork: true,
                overrideNames: overrideNames,
  ...
  if (getSelectedNpcs().length !== 0) {
    ...
    if (pikUpStiXModule?.active) {
      let treasurePosition = {
        ...
        gridY:
          getSelectedNpcs()[0].data.y - getSelectedNpcs()[0].scene.data.grid,
      };
